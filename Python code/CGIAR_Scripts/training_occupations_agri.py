...
def create_train_test(column_name, mapping):
    ...
    #load predicted gender data 
    gender_df = pd.read_csv('gender_prediction.csv')
    
    gender_df = gender_df.drop(['ground_truth'], axis=1)
    gender_df = gender_df[gender_df['prediction']==1]
    gender_df.set_index('MSISDN', inplace = True, drop = True)
    ...
    targets = targets[~pd.isna(targets[col])]
    ...
    data_retrain = data_df[data_df['index'].notin(gender['MSISDN'])]
    
    prediction_data =  data_df.merge(gender_df,left_on='index',right_on='MSISDN')
    
    
    prediction_data = prediction_data.drop(['index','prediction'], axis=1)
    prediction_data.set_index('MSISDN', inplace=True, drop=True)
    ...
    prediction_X = prediction_data.drop(['target'], axis=1)
    prediction_y = prediction_data['target'].copy()
    ...
    prediction_X = np.log(1 + prediction_X)
    ...
    return [X_train, y_train, X_test, y_test, prediction_X, prediction_y]
...
def main(max_evals):
    ...
    fname = './occupations_agri_results_{}.pickle'.format(dtime)
...
def get_hyperparams():
    # Fix random_state
    trials = pickle.load(open("occupations_agri_results_20191117-103453.pickle", "rb"))
    ...
    if results:
        ...
        return results[0]['result']['hyper'], results[0]['result']


# In[7]:


params, acc = get_hyperparams()
params
acc
...
def retrain(X_train, y_train, params):
...
X_train, y_train, X_test, y_test, prediction_X, prediction_y = create_train_test(column_name=col, mapping=mapping)

model = retrain(X_train, y_train, params)
model
...
def predict(model, X_test, y_test):
    ...
    return test_accuracy, test_df


test_accuracy, test_df = predict(model, X_test, y_test)
...
test_accuracy, test_df = predict(model, prediction_X, prediction_y )
test_accuracy
